...
(async () => {
    ...
    if(typeof(process.argv[2]) == 'undefined'){
        throw Error('Please provide the payer\'s account ID as first parameter');
    }
    ...
    console.log('Filling escrow slot with ID '+process.argv[3]+' from account '+process.argv[2]+' (address '+addr+')');
    await escrowPaymentSplitter.contract.methods.fillEscrowSlot(process.argv[3]).send({from: addr, gasLimit: 110000});
    console.log('Ok');
    Web3Helper.cleanup();
})().catch(error => {
